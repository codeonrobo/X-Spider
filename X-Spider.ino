    ...
    if (request->hasParam(PARAM_INPUT_1) && request->hasParam(PARAM_INPUT_2)) {
      ...
      if(inputMessage0.toInt()==1){
        ...
        if(angleSet == 0){
          ...
          if(activeServo == 0){InitAngle[activeServo] = preferences.putInt("PWM0", InitAngle[activeServo]);}
          else if(activeServo == 1){InitAngle[activeServo] = preferences.putInt("PWM1", InitAngle[activeServo]);}
          else if(activeServo == 2){InitAngle[activeServo] = preferences.putInt("PWM2", InitAngle[activeServo]);}
          else if(activeServo == 3){InitAngle[activeServo] = preferences.putInt("PWM3", InitAngle[activeServo]);}

          else if(activeServo == 4){InitAngle[activeServo] = preferences.putInt("PWM4", InitAngle[activeServo]);}
          else if(activeServo == 5){InitAngle[activeServo] = preferences.putInt("PWM5", InitAngle[activeServo]);}
          else if(activeServo == 6){InitAngle[activeServo] = preferences.putInt("PWM6", InitAngle[activeServo]);}
          else if(activeServo == 7){InitAngle[activeServo] = preferences.putInt("PWM7", InitAngle[activeServo]);}

          else if(activeServo == 8){InitAngle[activeServo] = preferences.putInt("PWM8", InitAngle[activeServo]);}
          else if(activeServo == 9){InitAngle[activeServo] = preferences.putInt("PWM9", InitAngle[activeServo]);}
          else if(activeServo == 10){InitAngle[activeServo] = preferences.putInt("PWM10", InitAngle[activeServo]);}
          else if(activeServo == 11){InitAngle[activeServo] = preferences.putInt("PWM11", InitAngle[activeServo]);}
        }
      }
...
void loop() {
  spiderMoveCtrl(globalLeft, globalRight);
  ...
  if (Serial.available() > 0){
    ...
    else if (incomedate == 0){
      globalLeft = 0;
      globalRight = 0;
    }
    else if (incomedate == 24){
      globalLeft = -1;
      globalRight = -1;
    }
    else if (incomedate == 102){
      globalLeft = -1;
      globalRight = 1;
    }
    else if (incomedate == 120){
      globalLeft =  1;
      globalRight = -1;
    }
  }
